...
// Project `cn` (clsx + tailwind-merge) rather than a plain join, so a caller's
// className actually overrides the defaults below instead of both landing on
...
export function SvgPathDrawingTextAnimation({
  ...
  return (
    <div
      className={cn(
        "flex min-h-[200px] w-full min-w-0 items-center justify-center",
        className,
      )}
    >
      <svg
        ref={svgRef}
        // Intrinsic size must match the viewBox: with `h-auto w-full` the box is
        // derived from these attributes, so a fixed 1000x420 letterboxed the
        // artwork inside ~240px of dead vertical space.
        width={viewBoxWidth}
        height={viewBoxHeight}
        viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`}
        // min-w-0: the width/height attrs give the svg an intrinsic 1000px,
        // which would otherwise set the flex min-content width and overflow
        // any narrower column it is dropped into.
        className="h-auto w-full min-w-0 max-w-full"
        role="img"
        aria-label={display}
